 ...
 *
 * @typedef {Object} PrecallTestOptions
 ...
 * @returns {Promise<void>}
 */
function _initialize(options, onResult) {
    return new Promise((resolve, reject) => {
        if (!options.disableThirdPartyRequests) {
            ...
            api.initialize(appId, appSecret, userId, (status, message) => {
                if (status === 'success') {
                    api.on('preCallTestResults', (testStatus, result) => {
 ...
 * @returns {Promise<void>}
 */
export async function init(options, onResult) {
    if (_initialized) {
    ...
    return _initialize(options, onResult);
}
